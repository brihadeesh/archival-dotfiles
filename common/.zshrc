...
%{$fg[yellow]%}[ %{$reset_color%}%~ %{$fg[yellow]%}]%{$fg[blue]%}
 └──────────|   % %{$reset_color%}%'
...
case $(basename "$(cat "/proc/$PPID/comm")") in
  login)
	RPROMPT="%{$fg[red]%} %(?..[%?])"
	alias x='startx ~/.xinitrc'      # Type name of desired desktop after x, xinitrc
...
if [ -f /usr/bin/fortune ]; then fortune -o | glitchcat; fi
